...
function App() {
  ...
  useEffect(() => {
    if (authReady && profile) {
      persistGuestWorkspace({ profile, expenses: expenseEntries, inventoryMeta: readInventoryMeta(), mode: isSupabaseAuthenticated ? 'authenticated' : 'guest' });
    }
  }, [expenseEntries, profile, authReady, isSupabaseAuthenticated]);
        ...
        <div className="space-y-4 sm:space-y-6">
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <article className={`rounded-2xl border p-5 shadow-lg ${isDarkMode ? 'border-slate-800 bg-slate-900/80 shadow-slate-950/30' : 'border-slate-200 bg-white/80 shadow-slate-200/70'}`}>
              ...
              <p className={`mt-3 text-2xl font-semibold ${expenseSummary.total === 0 ? (isDarkMode ? 'text-white' : 'text-slate-900') : (isDarkMode ? 'text-rose-400' : 'text-rose-600')}`}>{formatCurrency(expenseSummary.total)}</p>
            </article>
            <article className={`rounded-2xl border p-5 shadow-lg ${isDarkMode ? 'border-slate-800 bg-slate-900/80 shadow-slate-950/30' : 'border-slate-200 bg-white/80 shadow-slate-200/70'}`}>
